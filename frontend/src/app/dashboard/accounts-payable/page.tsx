...
interface AP { id: string; supplierName: string; ncfNumber?: string; issueDate: string; dueDate: string; amount: number; paidAmount: number; balance: number; status: string; }
...
export default function AccountsPayablePage() {
  ...
  const load = async () => {
    const [apRes, agingRes] = await Promise.all([
      api.get('/accounts-payable').catch(() => ({ data: { data: [] } })),
      api.get('/accounts-payable/aging').catch(() => ({ data: { data: null } })),
    ]);
    setItems(apRes.data.data ?? []);
  ...
  const registerPayment = async (id: string) => {
    try {
      await api.post(`/accounts-payable/${id}/payments`, { ...paymentForm, amount: parseFloat(paymentForm.amount) });
  ...
  return (
    <div>
      <PageHeader title="Cuentas por Pagar" description="Control de deudas con proveedores" />

      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-2"><span className="text-sm text-slate-500">Total por Pagar</span><DollarSign size={18} className="text-blue-500" /></div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.length === 0 && <tr><td colSpan={9} className="text-center py-8 text-slate-400">No hay cuentas por pagar</td></tr>}
            {items.map(ap => (
              <>
                <tr key={ap.id} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-700">{ap.supplierName}</td>
                  <td className="px-4 py-3 font-mono text-slate-500 text-xs">{ap.ncfNumber ?? '—'}</td>
                  <td className="px-4 py-3 text-slate-500">{formatDate(ap.issueDate)}</td>
                  <td className={`px-4 py-3 ${ap.status === 'OVERDUE' ? 'text-red-600 font-medium' : 'text-slate-500'}`}>{formatDate(ap.dueDate)}</td>
                  <td className="px-4 py-3 text-right">{formatCurrency(ap.amount)}</td>
                  <td className="px-4 py-3 text-right text-green-600">{formatCurrency(ap.paidAmount)}</td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-800">{formatCurrency(ap.balance)}</td>
                  <td className="px-4 py-3 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[ap.status] ?? 'bg-slate-100 text-slate-600'}`}>{statusLabels[ap.status] ?? ap.status}</span></td>
                  <td className="px-4 py-3 text-right">
                    {ap.status !== 'PAID' && ap.status !== 'CANCELLED' && (
                      <button onClick={() => setPayingId(payingId === ap.id ? null : ap.id)} className="text-blue-600 hover:text-blue-700 text-xs font-medium">Registrar Pago</button>
                    )}
                  </td>
                </tr>
                {payingId === ap.id && (
                  <tr key={`${ap.id}-pay`} className="bg-blue-50 border-b">
                    <td colSpan={9} className="px-4 py-3">
                      <div className="flex items-end gap-3">
                        <div><label className="block text-xs text-slate-500 mb-1">Monto</label><input type="number" step="0.01" max={ap.balance} className="w-32 border rounded px-2 py-1.5 text-sm" value={paymentForm.amount} onChange={e => setPaymentForm({ ...paymentForm, amount: e.target.value })} placeholder="0.00" /></div>
                        ...
                        <button onClick={() => registerPayment(ap.id)} className="bg-blue-600 text-white px-3 py-1.5 rounded text-sm hover:bg-blue-700">Guardar</button>
